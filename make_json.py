...
for ind, els in enumerate(data):
    ...
    if ind not in toDel:
        row = dict()
        row['id'] = els[0]
        row['transcription'] = els[2]
        row['pos'] = els[14].strip()
        if els[3]:
            row['pos'] = row['pos'] + ', ' + els[3] + ' class'
        row['audio'] = "<audio controls preload=\"none\"><source src=\"audio/index.wav\" type=\"audio/wav\"></audio>".replace('index', els[0])
        row['translation'] = els[10].split('_')[0]
        row['rustranslation'] = els[8].split('_')[0]
        if els[2] in adds:
            row['add'] = adds[els[2]]
            row['add_audio'] = ''
        res['data'].append(row)
